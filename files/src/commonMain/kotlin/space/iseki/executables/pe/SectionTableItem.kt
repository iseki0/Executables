...
data class SectionTableItem internal constructor(
    ...
    internal fun validate() {
        ...
        if (characteristics.value == 0) {
            throw PEFileException("Invalid section characteristics: no flags set", "section_name" to name)
        }

        // 检查节的虚拟大小和原始数据大小
        if (virtualSize == 0u && sizeOfRawData > 0u) {
            throw PEFileException(
                "Invalid section: virtual size is 0 but raw data size is not 0",
                "section_name" to name,
                "raw_data_size" to sizeOfRawData,
            )
        }
        ...
        if (sizeOfRawData > 0u) {
            if (pointerToRawData.value == 0u) {
                throw PEFileException(
                    "Invalid section: has raw data but pointer to raw data is 0",
                    "section_name" to name,
                )
            }
            ...
            if (pointerToRawData.value % 512u != 0u) {
                throw PEFileException(
                    "Invalid section: pointer to raw data is not aligned to file alignment boundary",
                    "section_name" to name,
                )
            }
        }
        ...
        if (numberOfRelocations > 0u && pointerToRelocations.value == 0u) {
            throw PEFileException(
                "Invalid section: has relocations but pointer to relocations is 0",
                "section_name" to name,
            )
        }
        ...
        if (numberOfLinenumbers > 0u && pointerToLinenumbers.value == 0u) {
            throw PEFileException(
                "Invalid section: has line numbers but pointer to line numbers is 0",
                "section_name" to name,
            )
        }
