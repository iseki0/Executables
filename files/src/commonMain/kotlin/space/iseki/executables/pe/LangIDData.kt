...
private val pattern = Regex("^0x([0-9A-Fa-b]+)\\t+([^,]+)")
private val ref = data.lineSequence().filter { it.isNotBlank() }.associate { line ->
    val (_, hex, name) = pattern.find(line)?.groupValues ?: error("Invalid line: $line")
    hex.toUShort(16) to name
}
private val rref = ref.entries.associateBy({ it.value }, { it.key })
...
value class LangID(val value: UShort) {
    constructor(value: String) : this(
        try {
            rref[value] ?: value.removePrefix("0x").toUShort(16)
        } catch (e: NumberFormatException) {
            throw IllegalArgumentException("Invalid LangID: $value")
        }
    )

    override fun toString(): String = ref[value] ?: "0x${value.toString(16)}"
