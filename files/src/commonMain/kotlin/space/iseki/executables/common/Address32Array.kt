...
value class Address32Array @PublishedApi internal constructor(private val storage: UIntArray) : Iterable<Address32> {
    val size: Int
        get() = storage.size
    ...
    fun containsAll(elements: Iterable<Address32>): Boolean {
        if (elements is Address32Array) {
            return storage.containsAll(elements.storage)
        }
        return elements.all { storage.contains(it.toUInt()) }
    }

    operator fun contains(element: Address32): Boolean = storage.contains(element.toUInt())
