...
value class Address64Array @PublishedApi internal constructor(private val storage: ULongArray) : Iterable<Address64> {
    val size: Int
        get() = storage.size
    ...
    fun containsAll(elements: Iterable<Address64>): Boolean {
        if (elements is Address64Array) {
            return storage.containsAll(elements.storage)
        }
        return elements.all { storage.contains(it.toULong()) }
    }

    operator fun contains(element: Address64): Boolean = storage.contains(element.toULong())

    operator fun get(index: Int) = storage.elementAt(index).toAddr()
}
