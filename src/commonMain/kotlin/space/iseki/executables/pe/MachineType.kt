...
value class MachineType(val rawValue: Short) {
    companion object {
        @JvmStatic
        fun valueOf(name: String): MachineType = when (name) {
            "UNKNOWN" -> UNKNOWN
            "AM33" -> AM33
            "AMD64" -> AMD64
            "ARM" -> ARM
            ...
            "WCEMIPSV2" -> WCEMIPSV2
                ...
                if (name.startsWith("UNKNOWN(") && name.endsWith(")")) {
                    MachineType(name.substring(8, name.length - 1).toShort())
                } else {
                    throw IllegalArgumentException("Unknown MachineType: $name")
                }
            }
        }
